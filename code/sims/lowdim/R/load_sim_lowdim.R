...
for (i in idx) { # currently don't have all 10,000, need longer time (e.g., 12 hours) for that
  ...
  if (null == 0) {
    ols_bias <- (rowMeans(out, na.rm = TRUE)[10] - rowMeans(out)[4])*sqrt(cur$n)
    ols_var <- apply(out, 1, function(x) var(unlist(x), na.rm = TRUE))[10]*cur$n
    ols_mse <- mean(apply(out, 2, function(x) (x[10] - rowMeans(out)[4])^2), na.rm = TRUE)*cur$n
    ols_sd <- sd(unlist(out[10,]), na.rm = TRUE)*sqrt(cur$n)
    ols_cover <- mean(apply(out, 2, function(x) x[11] <= rowMeans(out)[4] & x[12] >= rowMeans(out)[4]), na.rm = TRUE)
...
##########################################################
## PLOTS FOR LOCALLY CONSTANT LOESS SMOOTHING
##########################################################
#############################
