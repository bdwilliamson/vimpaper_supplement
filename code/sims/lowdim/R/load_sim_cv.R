...
vec <- 1:560
files <- as.list(paste("./output/sim_cv_null_", as.numeric(null), "_output_t_", vec, ".rds", sep = ""))
...
for (i in idx) {
  ...
  cover.o.lo <- mean(apply(out, 2, function(x) x[9] <= x[4] & x[10] >= x[4]))
...
## save it off
saveRDS(output.lo, paste0("output_loess_cv_null_", as.numeric(null), ".rds"))
##########################################################
...
png(paste0("plots/bias_vs_n_lowdim_cv_null_", as.numeric(null), ".png"), width = fig.width, height = fig.height, units = "px", res = 300)
...
## coverage
#############################
png(paste0("plots/cover_vs_n_lowdim_cv_null_", as.numeric(null), ".png"), width = fig.width, height = fig.height, units = "px", res = 300)
...
## mean squared error
#############################
png(paste0("plots/mse_vs_n_lowdim_cv_null_", as.numeric(null), ".png"), width = fig.width, height = fig.height, units = "px", res = 300)
...
## variance
#############################
png(paste0("plots/var_vs_n_lowdim_cv_null_", as.numeric(null), ".png"), width = fig.width, height = fig.height, units = "px", res = 300)
