...
do.one <- function(n, p = 2, null, j, h.f.l, h.m.l){
  
  ## generate the data
  dat <- gen_data(n, p, null)
  
  ## calculate the estimators
  estVec <- calculateEsts(dat$dat, dat$truth[j], n, j, h.f.l, h.m.l)
...
source("sim_ests_cv.R")
source("sim_helpers.R")

set.seed(current$seed)
system.time(out <- replicate(current$B, do.one(current$n, current$p, null, current$j, 
  h.f.l, h.m.l)))
saveRDS(out, file = paste0("sim_cv_null_", as.numeric(null),"_output_t_", job.id, ".rds"))