...
calculateEsts <- function(data, truth, n, j, h.f.l, h.m.l, b){
  ...
  for (l in 1:5) {
    ...
    fullmod <- get_mod_0(train_outer, 5, h.f.l)
    ...
    minusmod <- get_mod_0(small_train_outer_dat, 5, h.f.l)
    ...
    cv_est <- mean((preds_full - preds_red)^2, na.rm = TRUE)/mean((test_outer$y - mean(test_outer$y))^2)
    cv_ests[l] <- cv_est
    ...
    ses[l] <- sqrt(mean(vimp::vimp_update(preds_full, preds_red, test_outer$y, type = "regression", na.rm = TRUE)^2, na.rm = TRUE))# /sqrt(dim(test_outer)[1])
  }
