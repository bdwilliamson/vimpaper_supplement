###########################################################################################################
##
## FILE:    simulation_hidim_twostep_ests.py
##
...
def getNaives(dat, n = 1000, p = 15, j = None, mod_f = None, mod_s = None, std = bool('False')):
    ...
    naive = np.array([vi.variableImportance(full_fit, small_fit, y, n, std)[0]])
    return naive
...
def calculateEsts(dat, n = 1000, p = 15, j = None, truth = None, test = None, std = bool('True')):
    ...
    ests = vi.variableImportance(full_fit, small_fit, y, n, std)
    ...
    ## apply getNaives to the columns
    # naives = np.zeros((1, 100))
    # for i in range(0, 100):
    #     naives[:, i] = getNaives(boot[i], mod_f = full_mod, mod_s = small_mod, j = j, std = std)
    naives_2 = np.concatenate(naives, axis = 0)
    naive_ord = np.sort(naives_2)
    k = int(np.floor((naive_ord.size + 1)*0.025))
    # naive_ci = np.array([naive_ord[0, k - 1], naive_ord[0, naive_ord.size - k - 1]])
    naive_ci = np.array([naive_ord[k - 1], naive_ord[naive_ord.size - k - 1]])
    ## calculate standard error for one-step
    # onestep_se = onestepSE(y = y, f = full_fit, m = small_fit, n = naive)
    onestep_se = se.variableImportanceSE(full_fit, small_fit, y, n, std)
    
    ## calculate CI for one-step
    # onestep_ci = myConfInt(est = onestep, se = onestep_se, n = n, level = 0.95)
    onestep_ci = ci.variableImportanceCI(onestep, onestep_se, n, level = 0.95)
